...
static int openthread_init(struct net_if *iface)
{
	...
	LOG_INF("OpenThread Shell Initialized");

	if (IS_ENABLED(CONFIG_OPENTHREAD_COPROCESSOR)) {
		LOG_INF("CONFIG_OPENTHREAD_COPROCESSOR enabled");
		otPlatUartEnable();
		otNcpHdlcInit(ot_context->instance, ncp_hdlc_send);
	} else {
		LOG_INF("CONFIG_OPENTHREAD_COPROCESSOR disabled");
	...
	LOG_INF("CONFIG_OPENTHREAD_COPROCESSOR done");
